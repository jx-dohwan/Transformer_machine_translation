...
class MinimumRiskTrainingEngine(MaximumLikelihoodEstimationEngine):
    ...
    def _get_reward(y_hat, y, n_gram=6, method='gleu'):
        ...
        # detokenizer하고 blue를 계산해야하는데 그건 너무 느려서 tokenizer된 상태에서 계산할 것이다.
        with torch.no_grad():
            scores = []
    ...
    def train(engine, mini_batch):
        ...
        y_hat, indice = engine.model.search(
            x,
            is_greedy=False, # 강화학습은 랜덤샘플링을 통해서 잘하는것 이외에도 더 나은 솔루션이 있는지 탐색하기 위해 램덤샘플링으로 지정정
            max_length=engine.config.max_length
        ) # y_hat = (bs, m, |Vtgt|), indice = (bs, m) -> ont-hot 인덱스가 들어있을 것임
