...
class Attention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, dk=64):
        ...
        w = torch.bmm(Q, K.transpose(1, 2)) # (bs,n,hs) -> transpose (bs, hs, n) -> bmm : (bs,m,hs)*(bs,hs,n) = (bs,m,n)
        # |w| = (batch_size, m, n) -> 미니 배치의 각 샘플별, 디코더의 각 타임스텝별, 인코더의 각 타임스텝에 대한 어텐션 weight값  
        if mask is not None:
            assert w.size() == mask.size()
            w.masked_fill_(mask, -float('inf')) 

        w = self.softmax(w / (dk**.5)) # 스케일을 해주면 그래디언트가 좀 더 안정적으로 된다 합치면 1이되게 nomerize해준다.
        c = torch.bmm(w, V) # (bs, m, n) * (bs, n, hs) -> n은 날라간다. -> 원래는 (n_splits*bs, m, hs/n_splite) 밖에서 불러올때 
        # |c| = (batch_size, m, hidden_size) -> 미니배치의 각 샘플별, 디코더의 각 타임스텝별, 새롭게 얻어낸 컨텍스트벡터터
...
class MultiHead(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None): # decoder에 있는 self-attention은 mask가 필요하다. -> 미래의 ts을 보지못하게 하는 mask
        # |Q|    = (batch_size, m, hidden_size)
        ...
        c = c.split(Q.size(0), dim=0) # batch size만 쪼개준다.
        # |c_i| = (batch_size, m, hidden_size / n_splits)
        c = self.linear(torch.cat(c, dim=-1)) # 마지막 차원을 cat하여 붙인다. 그리고 linear에 통과시키낟.
...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn_norm = nn.LayerNorm(hidden_size) # 먼저 더하고 layerNorm하는 것이다.
        ...
        self.fc_norm = nn.LayerNorm(hidden_size)# feedforward다음에 있는 layernorm 
...
# 학습과 추론이 다르게 동작 학습은 전체, 추론은 한 타임씩
class DecoderBlock(nn.Module): # self-attention, encoder에 대한 attention
    ...
    def __init__(
        ...
        self.fc = nn.Sequential(
            nn.Linear(hidden_size, hidden_size * 4), # 4배로 불렸다가
            nn.LeakyReLU() if use_leaky_relu else nn.ReLU(),
            nn.Linear(hidden_size * 4, hidden_size), # 다시 돌아옴
        )
    ...
    def forward(self, x, key_and_value, mask, prev, future_mask):
        ...
        if prev is None: # Training mode prev는 추론모드
